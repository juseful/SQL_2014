...
SELECT TO_CHAR(A.ORDEPTDATE,'YYYY-MM-DD')||'('||TO_CHAR(A.ORDEPTDATE,'DY')||')' ORDEPTDATE
  ...
  FROM (/* 영상검사 */
        ...
        SELECT /* APCT, MRA */
               A.ORDEPTDATE, A.PATNO
             , CASE
                    WHEN A.APCT||A.MRA = '12' THEN 'APCT+MRA'
                    WHEN A.APCT||A.MRA = '1' THEN 'APCT'
                    WHEN A.APCT||A.MRA = '2' THEN 'MRA'
                    ELSE ''
               END EXAM
          FROM (-- 건진 예정자 전체 검사 예정
                SELECT -- A.*, B.ORDNAME
                       A.ORDEPTDATE, A.PATNO
                     , MAX(DECODE(A.ORDCODE,'RC3120','1')) APCT
                     , MAX(DECODE(A.ORDCODE,'RM1013N','2')) MRA
                  FROM (-- PKG 내 USERSET 제외항목
                        SELECT *
                          FROM (
                                SELECT A.ORDEPTDATE, A.PATNO, B.ORDCODE
...
-- 최종, 수진자
SELECT TO_CHAR(A.ORDDATE,'YYYY-MM-DD')||'('||TO_CHAR(A.ORDDATE,'DY')||')' ORDEPTDATE
     , A.PATNO
     , CASE WHEN LENGTH(SUBSTR(A.PATNO,1,5)||((789+SUBSTR(A.PATNO,6))||SUBSTR(A.PATNO,6))) = '11'
            THEN '0'||SUBSTR(A.PATNO,1,5)||((789+SUBSTR(A.PATNO,6))||SUBSTR(A.PATNO,6))
       ELSE SUBSTR(A.PATNO,1,5)||((789+SUBSTR(A.PATNO,6))||SUBSTR(A.PATNO,6))
  ...
  FROM (/* 영상검사 */
        SELECT A.ORDDATE, A.PATNO
          ...
          FROM (-- PKG 내 USERSET 제외항목
                SELECT *
                  FROM (
                        SELECT A.ORDDATE, A.PATNO, B.ORDCODE
                          FROM SMRSV00T A, SMPKGMDT B
                ...
                SELECT A.*
                  FROM (
                        SELECT A.ORDDATE, A.PATNO, B.PKGEXAMCODE
                          FROM SMRSV00T A, SMADD00T B
                         WHERE A.ORDDATE BETWEEN TO_DATE(&D1,'YYYYMMDD') AND TO_DATE(&D2,'YYYYMMDD')
                           AND A.CANCELTIME IS NULL
                           AND A.CONTNO LIKE '&CUSTCODE%'
